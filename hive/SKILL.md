...
3. 搜索时使用的标签必须遵守下方“标签规范”。
4. 输出为 Markdown，结果按命中类型分为 `Error Hits` 和 `Summary Hits`，同一分组内多条记录用 `---` 分隔。
5. 单条记录通常包含：
...
- 标签中只允许使用中文、字母、数字和规定符号，不允许其他符号，避免检索、匹配和人工维护时出现歧义。
- 规定符号说明：`_`用于替换空格，用于命令。`.`用于文件扩展名。
- `tags` 数组中的每个标签都不允许包含空格；若标签由多个词组成，统一使用 `_` 连接。
- 除专有名词、文件名、方法名等必要名词外，标签一律优先使用中文；仅在确实无法自然翻译，或现有数据与接口约定已明确要求英文时，才保留英文形式。
- 生成标签时尽量丰富，优先覆盖正文中的关键名词、文件名、方法名、模块名、业务对象、错误类型、命令、配置项与流程节点，避免只写过于泛化的标签。
- 搜索标签与写入标签都必须遵守同一套规范，避免同一主题在检索和落库时出现两套命名。
